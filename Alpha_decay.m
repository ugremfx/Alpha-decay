...
%no_seg = 100; % Number of segments the barrier is divided in Change this
no_seg = input('Enter the number of segments: \n');

%R0 = 1.01; % fm , given in assignment comments Change this, between 1.3 - 1.5
R0 = input('Enter the radius R0: \n');
AD = 234; % Mass number of Daughter nucleus Change this
%AD = input('Enter the mass number of Daughter nucleus: \n');
...
%mass_daughter = input('Enter the mass of Daughter atom in Mev/c^2: \n');
mass_parent = 238.05078826*931.4941024228; % MeV/c^2
%mass_parent = input('Enter the mass of parent atom in MeV/c^2: \n');

Z1 = 92; % Daughter nucleus. Change this
%Z1 = input('Enter the Z number (no. of protons) for Daughter nucleus: \n');
Z2 = 2;  % alpha particle
...
k(1) =  sqrt(2*mass_alpha*(E_kin-Veff(1)))/hbarc; % [fm^-1] This is the first k 
k(2:no_seg+1) = sqrt(2*mass_alpha*(Veff(2:end-1)-E_kin))/hbarc; % [fm^-1] These are the k's in between
k(no_seg+2) =  sqrt(2*mass_alpha*(E_kin-Veff(end)))/hbarc; % [fm^-1] This is the last k
